...
| `OnScreenAnnouncerProcessor.py` | `nifiapi.flowfiletransform.FlowFileTransform` | Posts a one-time "you're on screen N" announcement into a loaded streamer's own Twitch channel, on a FlowFile tapped off the dispatch-success signal (so it only fires once a screen load actually succeeded). Carries its own IRC connection machinery and refresh token, separate from the listener and joiner; skips `kick:` logins. `Dry Run` for safe testing. |
| `KickOnScreenAnnouncerProcessor.py` | `nifiapi.flowfiletransform.FlowFileTransform` | Kick twin of `OnScreenAnnouncerProcessor.py`: one channel lookup (app token) plus one `POST /public/v1/chat` (user token) to announce the on-screen load into the loaded streamer's Kick channel as the bot identity. Only handles `kick:` logins. `Dry Run` for safe testing. |
...
`XLivePostProcessor.py`, `TwitchChatListenerProcessor.py`, `TwitchChatReplyProcessor.py`, `WatchlistChatJoinerProcessor.py`, `OnScreenAnnouncerProcessor.py`, and `KickOnScreenAnnouncerProcessor.py` are the NiFi-side half of `cso-operator-app`'s Streamers module — the actual app logic, HTTP endpoints, and NiFi flow they plug into live in that repo, not here:
